...
def best_hyperparameter_testing(dataset, X_tensor, y_tensor, result_folder, device = 'cpu', n_steps=10):
    ...
    df_test = pd.DataFrame(columns = hyperparameter_setting_cols + val_cols + test_cols + time_cols, index = range((n_bits_max - n_bits_min + 1)*n_steps))
    df_test['bits'] = np.repeat(range(n_bits_min, n_bits_max + 1), n_steps)
    df_test['step'] = np.tile(range(n_steps), n_bits_max - n_bits_min + 1)
 
    for n_bits in range(n_bits_min, n_bits_max + 1):
        print(f"Processing {n_bits}-bit quantization")
        for step in tqdm(range(n_steps)):
            current_row = (n_bits - n_bits_min) * n_steps + step
            SEED = 42 + step
            ...
            df_test.loc[current_row, hyperparameter_setting_cols] = best_hyperparameter_ids.values
            df_test.loc[current_row, val_cols] = best_val_losses.values

            for method in methods:
                best_hyperparameter_id = df_test.at[current_row, 'best_hyperparameter_id_' + method]
                ...
                if method in ['FP', 'Po-MQ', 'Po-QQ']:
                    ...
                    if method == 'FP':
                        df_test.at[current_row, 'test_loss_FP'] = test_loss_FP
                    elif method == 'Po-MQ':
                        df_test.at[current_row, 'test_loss_Po-MQ'] = test_loss_PoMQ
                    elif method == 'Po-QQ':
                        df_test.at[current_row, 'test_loss_Po-QQ'] = test_loss_PoQQ
                    df_test.at[current_row, f'time_{method}'] = time_FP
                ...
                # Calculate losses for pre-training quantization model
                    ...
                    val_loss_PrMQ, val_loss_PrQQ, test_loss_PrMQ, test_loss_PrQQ, train_loss_PrMQ, train_loss_PrQQ, time_PrMQ = train_pre_model(train_loader=train_loader, val_loader=test_loader, test_loader=test_loader, architecture=architecture,
                        ...
                        n_bits=n_bits, device=device)
                    if method == 'Pr-MQ':
                        df_test.at[current_row, 'test_loss_Pr-MQ'] = test_loss_PrMQ
                    elif method == 'Pr-QQ':
                        df_test.at[current_row, 'test_loss_Pr-QQ'] = test_loss_PrQQ
                    df_test.at[current_row, f'time_{method}'] = time_PrMQ
                ...
                val_loss_llt4, val_loss_llt_training4, test_loss_llt4, test_loss_llt_training4, train_loss_llt4, time_llt4 = train_llt( train_loader=train_loader, val_loader=test_loader, test_loader=test_loader,
                    ...
                    if method == 'LLT9':
                        ...
                        df_test.at[current_row, 'test_loss_LLT4'] = test_loss_llt4
                        df_test.at[current_row, f'time_{method}'] = time_llt4    
                ...
                # Calculate losses for scalar, resp. bitwise, minmax, resp. quantile, quantization model
                    ...
                    if method == 'Bw-MQ':
                        ...
                        df_test.at[current_row, 'test_loss_Bw-QQ'] = test_loss_BwQQ
                        df_test.at[current_row, f'time_{method}'] = time_BwMQ    
            df_test.to_csv(f'{result_folder}/{dataset}/{dataset}_best_hyperparameter_test_multiple.csv', index=False)                
    df_test.to_csv(f'{result_folder}/{dataset}/{dataset}_best_hyperparameter_multiple.csv', index=False)
    return df_test                    
