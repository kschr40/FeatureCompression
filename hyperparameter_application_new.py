...
from training import train_fp_model, train_pre_model, train_SQ_mlp, train_soft_comp_mlp, train_hard_comp_mlp, train_llt, train_lsq, train_soft_plus_mlp
import argparse
import os
import re
import glob
...
def hyperparameter_application(X_tensor : torch.tensor, y_tensor : torch.tensor, result_folder, dataset,
    ...
    # ensure a dataset-specific subdirectory under 'results' exists
    dataset_results_dir = Path('results') / dataset
    dataset_results_dir.mkdir(parents=True, exist_ok=True)

    results_df = pd.DataFrame(index = list(range(1)))
    ...
    n = X_tensor.shape[0]
    indices = np.arange(n)
    train_idx, val_idx = train_test_split(indices, train_size=0.9, shuffle=True, random_state=42)
    fold = 0
    ...
    model_path = f'pretrained_models/{dataset}_mlp_model.pth'
    # Calculate losses for mlp model
    val_loss_mlp, val_loss_hard_post_mlp, val_loss_hard_thr_post_mlp, train_loss_mlp, time_mlp = train_fp_model(train_loader=train_loader, val_loader=val_loader,
        architecture=architecture, min_values=min_values, max_values=max_values, quantile_thresholds=thresholds,
        num_epochs=num_epochs, learning_rate=learning_rate, weight_decay=weight_decay,
        n_bits=n_bits, device=device, save_model_path=model_path)

    # Calculate losses for pre-training quantization model
    val_loss_hard_pre_mlp, val_loss_hard_thr_pre_mlp, train_loss_hard_thr_pre_mlp_mm, train_loss_hard_thr_pre_mlp_q, time_prMQ = train_pre_model(train_loader=train_loader, val_loader=val_loader, architecture=architecture, min_values=min_values, max_values=max_values, quantile_thresholds=thresholds,
        num_epochs=num_epochs, learning_rate=learning_rate, weight_decay=weight_decay,
        n_bits=n_bits, device=device, load_model_path=model_path)

    # Calculate losses for soft quantization model
    val_loss_soft_mlp, val_loss_soft_hard_mlp, train_loss_soft, time_SQ = train_SQ_mlp(train_loader=train_loader, val_loader=val_loader, architecture=architecture, min_values=min_values, max_values=max_values, quantile_thresholds=thresholds,
        num_epochs=num_epochs, learning_rate=learning_rate, weight_decay=weight_decay,
        n_bits=n_bits, decrease_factor=decrease_factor, device=device, load_model_path=model_path)
    ...
    results_df.to_csv(f'{result_folder}/{dataset}/hyperparameter_application_{n_bits}bits_pretrained.csv', index=False)
    return results_df
